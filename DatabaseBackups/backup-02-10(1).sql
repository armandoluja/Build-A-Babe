...
--
-- Host: 127.0.0.1
-- Generation Time: Feb 10, 2016 at 07:11 AM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getAllMessages`(IN `userId` INT(11))
    NO SQL
SELECT chat.chatId, message.senderId, message.receiverId, message.timeStamp, message.content FROM chat JOIN message on chat.chatId = message.chatId WHERE (chat.userId1 = userId OR chat.userId2 = userId) AND chat.status IS NULL ORDER by message.timeStamp DESC$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getChat`(IN `userIdFrom` INT(11), IN `userIdTo` INT(11))
    NO SQL
SELECT * from chat where userId1 = userIdFrom AND userId2 = userIDTo$$
...
(1, 1, '2016-02-10 05:38:18'),
(1, 2, '2016-02-10 05:48:43'),
(1, 3, '2016-02-10 06:00:50');
...
(1, 'tempuser', '99df00dac4e7820a4df1f49b3148b845513565e8', '00000000000000000000', 'QKi8iFydNzeHkS0UqLXjXjqakf7lQyzLRdSYK3GS', 0),
...
(1, 'Jack', 'Hallam', 'M', 1, 3, 1, 1, 3, 'This%20is%20the%20bio', '1996-05-26', 69, 69),
(3, 'arm', 'ando', 'M', 0, 1, 2, 3, NULL, 'arm bio', '1960-01-01', 10, 70),
(4, 'Joy', 'Gleeful', 'F', 2, 2, 3, 3, NULL, 'Hey%20im%20gleejohy', '1994-07-07', 22, 64),
(5, 'sam', 'sam', 'M', 0, 0, 0, 0, NULL, '', '1960-01-01', 10, 66),
